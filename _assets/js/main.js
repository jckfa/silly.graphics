...
function toggleMainList(index, size) {
  ...
  if (mainListContContainer[index].classList.contains(over800class)) {
    ...
    // mainListContContainer[index].style.height = "calc(" + size + " * (3.1rem + 1em))";
    // mainListContContainer[index].style.height = "calc(" + size + " * (" + bleh + " + 0.1rem))";
    // mainListContContainer[index].style.height = "calc(" + size + " * (2.8rem + (var(--target-area) * 2) + 0.1rem))";
